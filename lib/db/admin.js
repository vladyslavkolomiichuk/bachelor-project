...
export async function adminDeleteFromDb(type, itemId) {
  ...
  try {
    ...
    if (type === "users") {
      const userImageRes = await client.query(
        "SELECT image FROM users WHERE id = $1",
        [itemId]
      );
      const userImageUrl = userImageRes.rows[0]?.image;
      ...
      const bookImageRes = await client.query(
        "SELECT image FROM books WHERE id = $1",
        [itemId]
      );
      const bookImageUrl = bookImageRes.rows[0]?.image;

      const pdfRes = await client.query(
        `SELECT pdf_file_url, user_id 
         FROM used_books 
         WHERE book_id = $1 AND pdf_file_url IS NOT NULL`,
        [itemId]
      );

      for (const { user_id } of pdfRes.rows) {
        const folderPath = `books/${itemId}/${user_id}/`;
        ...
        if (listError) {
          console.error(`Error listing files for user ${user_id}:`, listError);
          continue; 
        }
        ...
        if (list && list.length > 0) {
          ...
          if (deleteError) {
            console.error(
              `Error deleting PDF files for user ${user_id}:`,
              deleteError
            );
          }
        }
      ...
      if (bookImageUrl) {
        await deleteImage(bookImageUrl);
      }
